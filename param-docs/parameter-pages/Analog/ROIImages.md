...
Master on/off switch for saving the per-dial **ROI images** (the small cut-out image of each analog ROI)
...
- **enabled / `true`**: ROI images are written to *ROI Images Location* (default `/log/analog`), kept for
  *ROI Images Retention* days.

This is the analog-ROI equivalent of [Save Raw Images](../TakeImage/RawImages.md). For back-compatibility,
