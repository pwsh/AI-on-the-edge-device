...
2. **Power management** (`esp_pm` DFS + tickless idle / light sleep). **Feasibility verdict (2026-06-01,
   code-grounded): ESP32-CAM ❌ not feasible; ESP32-S3 🟡 feasible candidate (needs hardware validation).
   Pursue PM on the S3 only.**
   - **Current state:** `CONFIG_PM_ENABLE=y` already, but `CONFIG_PM_DFS_INIT_AUTO` and
     ...
     WiFi modem-sleep (IDF default `WIFI_PS_MIN_MODEM`) is the only active saver today. The
     `power-management` build variant (`sdkconfig.defaults.power-management`, §9.1) flips those on.
   - **The decisive factor — the camera XCLK clock source (verified in the IDF SoC caps + esp32-camera
     v2.1.6 driver):** XCLK (20 MHz) is generated by LEDC (`xclk.c`, `LEDC_AUTO_CLK`) and the camera
     runs it **continuously**; the esp32-camera driver takes **no `esp_pm_lock`** (grep: zero hits on
     either target). So PM-safety depends entirely on whether the XCLK can be sourced from a clock that
     DFS/light-sleep does **not** scale:
     - **ESP32 (ESP32-CAM): no such clock.** `SOC_LEDC_SUPPORT_XTAL_CLOCK` is **undefined**; the ESP32
       LEDC only offers APB (80 MHz) and RC_FAST (~8 MHz, can't make a clean 20 MHz). So the XCLK
       **must** come from APB, which DFS scales with the CPU. Down-clocking → XCLK drifts → corrupted
       captures / bad reads. The only way to keep it clean is to pin APB at max for the whole uptime
       (`ESP_PM_APB_FREQ_MAX`/no-light-sleep continuously), which **negates the entire DFS/light-sleep
       benefit**. Net: either no power saving or broken images → **not feasible** (also why upstream
       quarantined PM in a separate `esp32cam-power-management` env that nobody runs by default).
     - **ESP32-S3: it has a DFS-independent source.** The S3 LEDC adds `LEDC_USE_XTAL_CLK`
       (`SOC_LEDC_SUPPORT_XTAL_CLOCK=1`); XTAL is 40 MHz → 40/2 = a clean 20 MHz XCLK that **does not
       move with CPU/APB DFS**, and the LEDC driver can keep XTAL powered through light sleep
       (`ledc_glb_clk_set_sleep_mode`, XPD). So the camera clock stays stable while the CPU/APB
       down-clock or enter automatic light sleep → DFS/light-sleep can coexist with the camera. The
       S3 also has more internal-RAM headroom (less light-sleep retention squeeze) and octal PSRAM
       with proper sleep power handling.
   - **Architecture limit (both targets):** this firmware is *always-on* (web server + MQTT +
     ~per-interval round), so deep-sleep-between-rounds (the big saver) isn't compatible without a
     redesign (wake → round → publish → sleep, dropping the always-reachable web UI). On the S3, DFS +
     automatic light-sleep keep connectivity but yield only **modest** savings on a Wi-Fi-connected
     device — worth doing, not transformative.
   - **S3 validation path (opt-in, on hardware):** build with `AIOTEDGE_VARIANTS=power-management`;
     (1) confirm the camera XCLK actually resolves to **XTAL** (LEDC `AUTO_CLK` selection, or force
     `LEDC_USE_XTAL_CLK`) — this is the make-or-break step; (2) DFS-only first
     (`esp_pm_configure({max=240, min=80MHz, light_sleep_enable=false})`), confirm CNN reads stay
     valid and images are clean across lighting; (3) then `light_sleep_enable=true` +
     `FREERTOS_USE_TICKLESS_IDLE`; (4) quantify with a **power meter** (not measurable remotely).
     Keep it an opt-in config/build option, never a forced default.
